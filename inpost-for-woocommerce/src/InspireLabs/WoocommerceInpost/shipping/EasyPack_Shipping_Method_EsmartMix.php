...
if ( ! class_exists( 'EasyPack_Shipping_Method_EsmartMix' ) ) {
	class EasyPack_Shipping_Method_EsmartMix extends EasyPack_Shippng_Parcel_Machines {
		...
		public static function ajax_create_package( $courier = false ) {
			...
			try {
				...
				\wc_get_logger()->debug( 'INPOST SmartCourier Exception: ', array( 'source' => 'inpost-smartcourier-log' ) );
		...
		public static function order_metabox_content(
			...
			if ( $shipment instanceof ShipX_Shipment_Model
				...
				$wrong_api_env = true;
				include 'views/html-order-metabox-courier-esmartmix.php';
				if ( ! $output ) {
					$out = ob_get_clean();
