	...
	<td class="forminp">
		<table id="<?php echo esc_attr( $field ); ?>" class="easypack_rates wc_input_table sortable widefat">
			<thead>
				<tr>
					<th class="sort">&nbsp;</th>
					<th class="price"><?php esc_html_e( 'Min', 'inpost-for-woocommerce' ); ?></th>
					<th class="price"><?php esc_html_e( 'Max', 'inpost-for-woocommerce' ); ?></th>
					<th class="price"><?php esc_html_e( 'Cost', 'inpost-for-woocommerce' ); ?></th>
					<th class="price"><?php esc_html_e( '%', 'inpost-for-woocommerce' ); ?>&nbsp;<span class="tips" data-tip="<?php esc_attr_e( 'The commission is added to the shipping cost according to the formula: [value of the cart] * [%] + [shipping cost].', 'inpost-for-woocommerce' ); ?>">[?]</span></th>
					<th class="action"><?php esc_html_e( 'Action', 'inpost-for-woocommerce' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php $count = 0; ?>
                    ...
                    <?php foreach ( $rates as $key => $rate) : $count++; ?>
                        <?php if( is_array( $rate ) ): ?>
                            <tr>
                                ...
                                <td class="action">
                                    <a id="delete_rate_<?php echo esc_attr( $count ); ?>" href="#" class="button delete_rate" data-id="<?php echo esc_attr( $count ); ?>"><?php esc_html_e( 'Delete row', 'inpost-for-woocommerce' ); ?></a>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php endforeach; ?>
			...
			<tfoot>
				<tr>
					<th colspan="6">
						<a id="insert_rate" href="#" class="button plus insert"><?php esc_html_e( 'Insert row', 'inpost-for-woocommerce' ); ?></a>
					</th>
				</tr>
			</tfoot>
		</table>
		<script type="text/javascript">
			function append_row( id ) {
				var code = '<tr class="new">\
								...
								<td>\
									<a id="delete_rate_'+id+'" href="#" class="button delete_rate" data-id="'+id+'"><?php esc_html_e( 'Delete row', 'inpost-for-woocommerce' ); ?></a>\
								</td>\
							</tr>';
			...
			jQuery(document).ready(function() {
				...
				jQuery(document).on('click', '.delete_rate',  function() {
					if (confirm('<?php esc_html_e( 'Are you sure?' , 'inpost-for-woocommerce' ); ?>')) {
