...
if ( ! class_exists( 'EasyPack_Shipping_Method_Courier_C2C_COD' ) ) {
	class EasyPack_Shipping_Method_Courier_C2C_COD extends EasyPack_Shippng_Parcel_Machines {
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->method_title       = esc_html__( 'InPost Courier C2C COD', 'inpost-for-woocommerce' );
			$this->method_description = esc_html__( 'InPost Courier C2C COD', 'inpost-for-woocommerce' );
			$this->init();
		}
		...
		public function init_form_fields() {
			...
			$settings = array(
				...
				'title'                                  => array(
					...
					'type'              => 'text',
					'default'           => __( 'InPost Courier C2C COD', 'inpost-for-woocommerce' ),
				...
				'default_package_size_c2c'               => array(
					'title'   => __( 'Default package size Courier C2C', 'inpost-for-woocommerce' ),
					'type'    => 'select',
					'class'   => 'wc-enhanced-select',
					'default' => 'small',
		...
		public static function order_metabox_content(
			...
			if ( $shipment instanceof ShipX_Shipment_Model
				...
				$wrong_api_env = true;
				include 'views/html-order-metabox-parcel-courier-c2c_cod.php';
				if ( ! $output ) {
					$out = ob_get_clean();
