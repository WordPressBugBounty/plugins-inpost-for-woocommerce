...
if ( ! class_exists( 'EasyPack_Shipping_Method_Courier_Local_Express_COD' ) ) {
	class EasyPack_Shipping_Method_Courier_Local_Express_COD extends EasyPack_Shippng_Parcel_Machines {
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->id           = 'easypack_shipping_courier_le_cod';
			$this->method_title = __( 'InPost Courier Local Express COD', 'inpost-for-woocommerce' );
			$this->init();
		}
		...
		public static function ajax_create_package( $courier = false ) {
			...
			if ( is_array( $cod_amounts ) ) {
				...
				if ( $total_amount != $order->get_total() ) {
					$ret['status']  = 'error';
					$ret['message'] = sprintf(
					/* translators: %1$s: Order total amount, %2$s: Total COD (Cash on Delivery) amounts */
						__(
