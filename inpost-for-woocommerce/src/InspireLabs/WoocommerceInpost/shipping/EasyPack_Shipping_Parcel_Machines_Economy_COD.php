...
if ( ! class_exists( 'EasyPack_Shipping_Parcel_Machines_Economy_COD' ) ) {
	class EasyPack_Shipping_Parcel_Machines_Economy_COD extends EasyPack_Shippng_Parcel_Machines {
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->id           = 'easypack_parcel_machines_economy_cod';
			$this->method_title = __( 'InPost Locker Economy COD', 'inpost-for-woocommerce' );
			$this->init();
		...
		public function init_form_fields() {
			...
			$settings = array(
				...
				'title'                                  => array(
					...
					'type'              => 'text',
					'default'           => __( 'InPost Locker Economy COD', 'inpost-for-woocommerce' ),
		...
		public static function order_metabox_content(
			...
			$geowidget_config = ( new Geowidget_v5() )->get_pickup_delivery_configuration( 'easypack_parcel_machines_economy_cod' );
			...
			if ( $shipment instanceof ShipX_Shipment_Model
				...
				$wrong_api_env = true;
				include 'views/html-order-matabox-parcel-machines-economy-cod.php';
				if ( ! $output ) {
					$out = ob_get_clean();
