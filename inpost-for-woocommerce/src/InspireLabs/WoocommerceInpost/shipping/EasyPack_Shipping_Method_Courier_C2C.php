...
if ( ! class_exists( 'EasyPack_Shipping_Method_Courier_C2C' ) ) {
	class EasyPack_Shipping_Method_Courier_C2C
		extends EasyPack_Shippng_Parcel_Machines {

		const WP_AJAX_ACTION_CREATE = 'courier_c2c_create_package';
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->method_title = __( 'InPost Courier C2C', 'woocommerce-inpost' );
			$this->init();
		}
		...
		public function init_form_fields() {
			...
			$settings = [
				...
				'title'              => [
					...
					'type'              => 'text',
					'default'           => __( 'InPost Courier C2C', 'woocommerce-inpost' ),
		...
		public static function order_metabox_content(
			...
			if ( $shipment instanceof ShipX_Shipment_Model
				...
				$wrong_api_env = true;
				include( 'views/html-order-matabox-parcel-courier-c2c.php' );
				if ( ! $output ) {
					$out = ob_get_clean();
