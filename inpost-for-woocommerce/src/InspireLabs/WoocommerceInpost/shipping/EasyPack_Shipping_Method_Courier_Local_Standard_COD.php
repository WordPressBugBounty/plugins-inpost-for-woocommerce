...
if ( ! class_exists( 'EasyPack_Shipping_Method_Courier_Local_Standard_COD' ) ) {
	class EasyPack_Shipping_Method_Courier_Local_Standard_COD extends EasyPack_Shippng_Parcel_Machines {
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->method_title = __( 'InPost Courier Local Standard COD', 'inpost-for-woocommerce' );
			$this->init();
		}
		...
		public function init_form_fields() {
			...
			$settings = array(
				...
				'flat_rate'                              => array(
					...
					'type'    => 'checkbox',
					'label'   => __(
						'Set a flat-rate shipping fee for the entire order.',
						'inpost-for-woocommerce'
					),
		...
		public static function ajax_create_shipment_model() {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'easypack_bulk_create_shipments' ) {
				...
				$parcels = ! empty( $parcels ) ? $parcels : array( get_option( 'easypack_default_package_size' ) );
		...
		public static function ajax_create_package( $courier = false ) {
			...
			if ( is_array( $cod_amounts ) ) {
				...
				if ( $total_amount != $order->get_total() ) {
					$ret['status']  = 'error';
					$ret['message'] = sprintf(
					    /* translators: 1: order total amount, 2: sum of COD amounts */
						__(
							'Order total %1$s do not equals total COD amounts %2$s.',
							'inpost-for-woocommerce'
						),
						$order->get_total(),
						$total_amount
